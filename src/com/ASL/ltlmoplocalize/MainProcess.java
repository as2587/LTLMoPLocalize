...
*/
public class MainProcess extends Activity implements CvCameraViewListener2, OnTouchListener{
	...
	private int idx, UDP_PORTNUM, iContourAreaMin = 1000, iLineThickness = 3, contOfInterest, touchFlag,
			movement_threshold = 50, touch_thresh = 50, robot_threshold = 50, locked=0;
	
	private double val, sq_x, sq_y, base_x, base_y, sq_rat, base_rat, xOffset, yOffset, x_mod, y_mod, x_touch, y_touch;
			...
			public void onClick(View v){
				if (locked==0){
					if(markers.allMarkDefined()>0){
						...
						Toast.makeText(getApplicationContext(), "All Markers Not Defined", Toast.LENGTH_SHORT).show();
					}
				}
				else if(locked==1){
	...
	public boolean onCreateOptionsMenu(Menu menu) {
	     ...
	     Log.d("MyActivity", ""+mResolutionList.size());
	...
	public void onCameraViewStarted(int width, int height) {
        ...
        //c1Detector.setHsvColor(new Scalar(49, 229, 221, 0));
        c1Detector.setHsvColor(markerHSV);
        
        c2Detector = new ColorBlobDetector(2);
        //c2Detector.setHsvColor(new Scalar(151, 255, 174, 0));
		...
		markers = new All_ID(movement_threshold, robot_threshold, wifiManager, UDP_IP_ADDRESS, UDP_PORTNUM);
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        c1_contours = c1Detector.getContours();
        c2_contours = c2Detector.getContours();
    	...
    	for (idx = 0; idx < c1_contours.size(); idx++) {
    			...
    			if (c1_contours.get(idx).rows() == 4) {
    				contOfInterest = contOfInterest+1;
    				Converters.Mat_to_vector_Point2f(c1_contours.get(idx), pts);
   			
    				Imgproc.drawContours(mRgba, c1_contours, idx, colorRed, iLineThickness);

    				Core.line(mRgba, pts.get(0), pts.get(2), colorRed, iLineThickness);
    				Core.line(mRgba, pts.get(1), pts.get(3), colorRed, iLineThickness);
    				
    				line_Intersec = findIntersec(pts.get(0), pts.get(2), pts.get(1), pts.get(3));
  
    				//If markers have previously been ID'd, the following code matches current found contours to the previously ID'd contours
    				if(!(markers.allMarkers.isEmpty())){
    					int temp = markers.closeToMarker(line_Intersec);
    					if (temp>=0){
    						markers.allMarkers.get(temp).setPoint(line_Intersec);
    						markers.allMarkers.get(temp).setLive();
    					}

    				}
    				if(touchFlag ==1){
    					if(dist(line_Intersec, lastClick)<touch_thresh){ //This ensures touch is near SOME valid contour of interest
    	...
    	for (idx = 0; idx < c2_contours.size(); idx++) {
    			...
    			if (c2_contours.get(idx).rows() == 4) {
    				...
    				Imgproc.drawContours(mRgba, c2_contours, idx, colorRed, iLineThickness);
    				...
    				if(touchFlag ==1){
    					if(dist(line_Intersec, lastClick)<touch_thresh){ //This ensures touch is near SOME valid contour of interest
    																// Contour of Interest may or may not be in allMarkers
    						Log.d("MyActivity", "Touched Near Robot Tag");
    						markers.incrementRb(line_Intersec);
    						touchFlag=0;
    					}
    				}//end if(touchFlag)
    			}
    			else if (c2_contours.get(idx).rows()==3){// end if(contour is 4sided)
    				contOfInterest = contOfInterest+1;
    				Converters.Mat_to_vector_Point2f(c2_contours.get(idx), pts);
    	...
    	Log.d("Switch", "Markers size: " + markers.allMarkers.size());
    	for (int i = 0; i < markers.allMarkers.size(); i++){
    		addNum(""+markers.allMarkers.get(i).getID(), markers.allMarkers.get(i).getPoint());
    	}
    	
    	for (int i = 0; i < markers.allRobots.size(); i++){
    		addNum(""+markers.allRobots.get(i).getID(), markers.allRobots.get(i).getPoint());
    	}
    	
    	if(!(markers.allMarkDefined()>0)){
    		markers.resetPass(1);
    		Log.d("MyActivity", "markers not defined");}
    	
		if(!(markers.allRobotDefined()>0))
			{markers.resetPass(2);
			Log.d("MyActivity", "robots not defined");}
		
		if((markers.allRobotDefined()>0)&&(markers.allMarkDefined()>0)){
			Log.d("MyActivity", "all defined!");
			bindAnchors();
			markers.sendInfo(1);
		}
		else{
	...
	public void bindAnchors(){
		for (int j = 0; j<4; j++){
			...
			markers.allMarkers.get(markers.pass_m[j]).setAnchor(new Point(Double.parseDouble(vals[0]),Double.parseDouble(vals[1])));
		}
	}
    ...
    private void addNum(String s, Point tagLoc){
    	Core.putText(mRgba, s, tagLoc, Core.FONT_HERSHEY_SIMPLEX, 1.2, colorBlue, 2);
    }

    private void loadPref(){
		SharedPreferences mySharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		RobotColor = mySharedPreferences.getString("RobotColor", "151,255,174,0");
		MarkerColor = mySharedPreferences.getString("MarkerColor", "49,229,221,0");
		markerHSV = conv2Scalar(MarkerColor);
		robotHSV = conv2Scalar(RobotColor);
		
		UDP_IP_ADDRESS = mySharedPreferences.getString("UDP_ip_add", "192.169.10.1");
		UDP_PORTNUM = Integer.parseInt(mySharedPreferences.getString("UDP_port_num", "5005"));
		
		markerinfo[0]= mySharedPreferences.getString("marker_1", "0,0");
		markerinfo[1]= mySharedPreferences.getString("marker_2", "0,10");
		markerinfo[2]= mySharedPreferences.getString("marker_3", "10,10");
		markerinfo[3]= mySharedPreferences.getString("marker_4", "10,0");

    }
    
    private Scalar conv2Scalar(String input){
    	String[] vals = input.split(",");
    	Log.d("MyActivity","Vals 1: "+ Double.parseDouble(vals[0]));
    	return (new Scalar(Float.parseFloat(vals[0]), Float.parseFloat(vals[1]), Float.parseFloat(vals[2]), Float.parseFloat(vals[3])));
